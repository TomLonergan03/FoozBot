...
<x-layout>
    ...
    <div class = 'flex items-center justify-center' >
        <form method = 'POST' action ='/register' onsubmit="submit.disabled = true; return true;" class="p-4 bg-gradient-to-bl from-stone-300 to-stone-400 rounded-xl border border-slate-500 shadow-lg text-center">
            @csrf

            <label for='first_name' class = ''> Username: </label><br>
            <input type = 'text' value = "{{old('first_name')}}" name = 'username' id = 'username'  required class = 'p-2 px-6 border-2 border-gray-500 rounded-xl shadow-lg'><br>
            @error('username')
            ...
            <a href="/login" class="text-slate-700 border border-slate-500 p-2 rounded-xl text-xs bg-FoozbotLBlue shadow-lg"><i>Or sign-in here</i></a>
